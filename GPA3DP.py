...
class GPA3DP:
	...
	def staticUpdateSymmetry(mods,gx,gy,gz,dists,maxGrad,tol):
		...
		for position, mod in np.ndenumerate(mods):
			if mod<tol:
				unknownP[position] = 1
			x,y,z = gx[position],gy[position],gz[position]
			d = dists[position]
			sumup = np.sqrt((gx+x)**2+(gy+y)**2+(gz+z)**2)/maxGrad 
	...
	def _G1B(self, symm):
		...
		if self.n_points < 3:
			...
			neigh = self.triangles.vertex_neighbor_vertices
			self.n_edges = len(neigh[1])/2
			self.G1B = np.log(float(self.n_points))/np.log(float(self.n_edges))
		if self.G1<0.0:
			self.G1B = 0.0
	...
	def _G3(self,symm):
		...
		if nterms>0.0:
			...
			alinhamento = 0.0
		if np.sum(opositeMat)+np.sum(targetMat)> 0:
			self.G3 = (float(np.sum(targetMat))/float(np.sum(opositeMat)+np.sum(targetMat)) ) + alinhamento
		else: 
			self.G3 = 0.0
	...
	def _eval(self,mat,moment=["G2"],symmetrycalGrad='A',showTimer=False):
		...
		for gmoment in moment:
			...
			if("G1" == gmoment):
				...
				retorno["G1"] = self.G1
			if("G1B" == gmoment):
				self._G1B(symmetrycalGrad)
				retorno["G1B"] = self.G1B
		if showTimer:
