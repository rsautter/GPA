...
def recursiveGN(inputMatrix, gn, tol, rad_tol, maxDepth, glist=[],level=0):
   ...
   if(maxDepth>level and  inputMatrix.shape[0]>2 and inputMatrix.shape[1]>2):
      ...
      gn2ul = runGPA(ul,tol,rad_tol,gn)
      gn2ur = runGPA(ur,tol,rad_tol,gn)
      gn2ll = runGPA(ll,tol,rad_tol,gn)
      gn2lr = runGPA(lr,tol,rad_tol,gn)   
      gn1 = runGPA(inputMatrix,tol,rad_tol,gn)
      glist.append([level,gn1-gn2ul])
      glist.append([level,gn1-gn2ur])
      glist.append([level,gn1-gn2ll])
      glist.append([level,gn1-gn2lr])      
...
def quadGPA(inputMatrix, gn, tol, rad_tol, levels=10):
...
def multipleFiles(fileName, gn, tol, rad_tol):
   ...
   np.savetxt(outputFile,save,header="file,Gn,alpha,beta,gamma",fmt="%s")
   print("Result save at "+outputFile)
